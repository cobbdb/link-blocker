...
function loadDoc(text) {
    ...
    document.body.innerHTML = newdoc.head.innerHTML;
    ...
    function refreshBase() {
        ...
        if (!isStatic(location.pathname)) {
            base.href += '/';
        }
        console.log('~~~~~ base is now', base.href);
    }
...
function sameOrigin(origin) {
    return origin.indexOf(location.origin) >= 0;
}
        ...
        function rebindAnchors() {
            ...
            for (i = 0; i < set.length; i += 1) {
                ...
                if (el.href) {
                    if (isRelative(href) || sameOrigin(el.origin)) {
                        el.addEventListener(
                            'click',
                            ClickHandler(el)
                        );
...
if (!window.running) {
    ...
    addEventListener('popstate', function (e) {
        ...
        //document.documentElement.innerHTML = e.state.page;
